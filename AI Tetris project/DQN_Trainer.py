...
def main ():
    ...
    run = wandb.init(
        ...
        config={
            ...
            "device": str(device),
            "state": "state = np.concatenate([col_height,[ bumpiness, holes]]).astype(np.float32)",
            "reward": "reward = lines_cleared * 1 - holes * 0.5 - bumpiness * 0.1  - height * 0.03",
    ...
    for epoch in range(epochs):
        ...
        while not done:
            ...
            reward = env.reward(state_dqn, after_state_dqn, done, state.cleared)
            moves += 1
