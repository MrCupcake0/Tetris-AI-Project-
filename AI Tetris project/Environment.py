...
class Environment:
    ...
    def draw(self, state, action):
        ...
        if type(action) == tuple:
            ...
            graphics(state, temp)
            state.cleared = self.clear_line(state.board)
            state.new_piece(state.board, self.screen)
        else:
    ...
    def reward(self, state, after_state, end_of_game, cleared):
        if end_of_game:
            return -100.0  
        
        lines_cleared = np.power(cleared, 2.0) 

        height = np.sum(after_state[0:10] - state[0:10])
        bumpiness = after_state[10] - state[10]
        holes = after_state[11] - state[11]
        # max_col = after_state[12] - state[12]

        reward = lines_cleared * 1 - holes * 0.5 - bumpiness * 0.1  - height * 0.03 
